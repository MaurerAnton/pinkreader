 ...
 * GPLv3 License
 * File: history_view.cpp - Implementation of Browsing history view
 */
...
HistoryView::HistoryView(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("UI::HistoryView", "Creating view...");

    setObjectName(QStringLiteral("HistoryView"));
    setAttribute(Qt::WA_DeleteOnClose, false);

    setupUi();
    connectSignals();

    Logging::debug("UI::HistoryView", "View created");
}

HistoryView::~HistoryView()
{
    Logging::debug("UI::HistoryView", "Destroying view...");
}

void HistoryView::activate()
{
    if (m_isActive) {
        Logging::debug("UI::HistoryView", "View already active");
        return;
    }

    Logging::info("UI::HistoryView", "Activating view...");
    m_isActive = true;
    ...
    Logging::info("UI::HistoryView", "View activated");
}
...
void HistoryView::deactivate()
{
    ...
    Logging::debug("UI::HistoryView", "Deactivating view...");
    m_isActive = false;
}

void HistoryView::refresh()
{
    Logging::debug("UI::HistoryView", "Refreshing view...");
    m_isEmpty = true;
    m_hasError = false;
    loadContent();
}
...
{
    return QStringLiteral("history_view");
}

QIcon HistoryView::icon() const
{
    return QIcon();
}

void HistoryView::showLoading()
{
    Logging::debug("UI::HistoryView", "Showing loading indicator");
    m_isLoading = true;
...
void HistoryView::showError(const QString &message)
{
    Logging::error("UI::HistoryView",
...
void HistoryView::showEmptyState(const QString &message)
{
    Logging::debug("UI::HistoryView",
...
bool HistoryView::isLoading() const
{
    return m_isLoading;
}

bool HistoryView::hasError() const
{
    return m_hasError;
}

bool HistoryView::isEmpty() const
{
    return m_isEmpty;
}

void HistoryView::closeEvent(QCloseEvent *event)
{
    Logging::debug("UI::HistoryView", "Close event received");
    deactivate();
...
{
    Logging::debug("UI::HistoryView", "Setting up UI...");
    ...
    m_toolbar->setObjectName(QStringLiteral("HistoryViewToolbar"));
    ...
    connect(backAction, &QAction::triggered,
            this, &HistoryView::backRequested);
    ...
    Logging::debug("UI::HistoryView", "UI setup complete");
}

void HistoryView::connectSignals()
{
    Logging::debug("UI::HistoryView", "Connecting signals...");
    ...
    connect(&Preferences::instance(), &Preferences::settingChanged,
        ...
        if (key == QStringLiteral("theme")) {
            Logging::debug("UI::HistoryView", "Theme changed, updating...");
            ThemeManager::instance().applyToWidget(this);
        }
    });
    ...
    Logging::debug("UI::HistoryView", "Signals connected");
}

void HistoryView::loadContent()
{
    Logging::debug("UI::HistoryView", "Loading content...");
    showLoading();
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        m_isEmpty = false;
        emit contentLoaded();
        Logging::debug("UI::HistoryView", "Content loaded successfully");
    });
}
