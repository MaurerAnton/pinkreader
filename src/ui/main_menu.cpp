 ...
 * GPLv3 License
 * File: main_menu.cpp - Implementation of Main sidebar navigation menu
 */
...
MainMenu::MainMenu(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("UI::MainMenu", "Creating view...");
    ...
    Logging::debug("UI::MainMenu", "View created");
}

MainMenu::~MainMenu()
{
    Logging::debug("UI::MainMenu", "Destroying view...");
}

void MainMenu::activate()
{
    if (m_isActive) {
        Logging::debug("UI::MainMenu", "View already active");
        return;
    }

    Logging::info("UI::MainMenu", "Activating view...");
    m_isActive = true;
    ...
    Logging::info("UI::MainMenu", "View activated");
}
...
void MainMenu::deactivate()
{
    ...
    Logging::debug("UI::MainMenu", "Deactivating view...");
    m_isActive = false;
}

void MainMenu::refresh()
{
    Logging::debug("UI::MainMenu", "Refreshing view...");
    m_isEmpty = true;
    m_hasError = false;
    loadContent();
}
...
{
    return QStringLiteral("main_menu");
}

QIcon MainMenu::icon() const
{
    return QIcon();
}
...
{
    Logging::debug("UI::MainMenu", "Showing loading indicator");
    m_isLoading = true;
...
void MainMenu::showError(const QString &message)
{
    Logging::error("UI::MainMenu",
...
void MainMenu::showEmptyState(const QString &message)
{
    Logging::debug("UI::MainMenu",
...
bool MainMenu::isLoading() const
{
    return m_isLoading;
}

bool MainMenu::hasError() const
{
    return m_hasError;
}
...
{
    Logging::debug("UI::MainMenu", "Close event received");
    deactivate();
...
{
    Logging::debug("UI::MainMenu", "Setting up UI...");
    ...
    m_toolbar->setObjectName(QStringLiteral("MainMenuToolbar"));
    ...
    Logging::debug("UI::MainMenu", "UI setup complete");
}

void MainMenu::connectSignals()
{
    Logging::debug("UI::MainMenu", "Connecting signals...");
    ...
    connect(&Preferences::instance(), &Preferences::settingChanged,
        ...
        if (key == QStringLiteral("theme")) {
            Logging::debug("UI::MainMenu", "Theme changed, updating...");
            ThemeManager::instance().applyToWidget(this);
        }
    });
    ...
    Logging::debug("UI::MainMenu", "Signals connected");
}

void MainMenu::loadContent()
{
    Logging::debug("UI::MainMenu", "Loading content...");
    showLoading();
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        m_isEmpty = false;
        emit contentLoaded();
        Logging::debug("UI::MainMenu", "Content loaded successfully");
    });
}
