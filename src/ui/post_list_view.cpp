 ...
 * GPLv3 License
 * File: post_list_view.cpp - Implementation of Post listing and browsing view
 */
...
PostListView::PostListView(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("UI::PostListView", "Creating view...");

    setObjectName(QStringLiteral("PostListView"));
    setAttribute(Qt::WA_DeleteOnClose, false);

    setupUi();
    connectSignals();

    Logging::debug("UI::PostListView", "View created");
}

PostListView::~PostListView()
{
    Logging::debug("UI::PostListView", "Destroying view...");
}

void PostListView::activate()
{
    if (m_isActive) {
        Logging::debug("UI::PostListView", "View already active");
        return;
    }

    Logging::info("UI::PostListView", "Activating view...");
    m_isActive = true;
    ...
    Logging::info("UI::PostListView", "View activated");
}

void PostListView::deactivate()
{
    if (!m_isActive) {
        return;
    }

    Logging::debug("UI::PostListView", "Deactivating view...");
    m_isActive = false;
}

void PostListView::refresh()
{
    Logging::debug("UI::PostListView", "Refreshing view...");
    m_isEmpty = true;
    m_hasError = false;
    loadContent();
}

QString PostListView::title() const
{
    return QStringLiteral("post_list_view");
}

QIcon PostListView::icon() const
{
    return QIcon();
}

void PostListView::showLoading()
{
    Logging::debug("UI::PostListView", "Showing loading indicator");
    m_isLoading = true;
...
void PostListView::showError(const QString &message)
{
    Logging::error("UI::PostListView",
...
void PostListView::showEmptyState(const QString &message)
{
    Logging::debug("UI::PostListView",
...
bool PostListView::isLoading() const
{
    return m_isLoading;
}

bool PostListView::hasError() const
{
    return m_hasError;
}

bool PostListView::isEmpty() const
{
    return m_isEmpty;
}

void PostListView::closeEvent(QCloseEvent *event)
{
    Logging::debug("UI::PostListView", "Close event received");
    deactivate();
    QWidget::closeEvent(event);
}

void PostListView::setupUi()
{
    Logging::debug("UI::PostListView", "Setting up UI...");
    ...
    m_toolbar->setObjectName(QStringLiteral("PostListViewToolbar"));
    ...
    connect(backAction, &QAction::triggered,
            this, &PostListView::backRequested);
    ...
    Logging::debug("UI::PostListView", "UI setup complete");
}

void PostListView::connectSignals()
{
    Logging::debug("UI::PostListView", "Connecting signals...");
    ...
    connect(&Preferences::instance(), &Preferences::settingChanged,
        ...
        if (key == QStringLiteral("theme")) {
            Logging::debug("UI::PostListView", "Theme changed, updating...");
            ThemeManager::instance().applyToWidget(this);
        }
    });
    ...
    Logging::debug("UI::PostListView", "Signals connected");
}

void PostListView::loadContent()
{
    Logging::debug("UI::PostListView", "Loading content...");
    showLoading();
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        m_isEmpty = false;
        emit contentLoaded();
        Logging::debug("UI::PostListView", "Content loaded successfully");
    });
}
