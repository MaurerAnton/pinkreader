 ...
 * GPLv3 License
 * File: image_viewer.cpp - Implementation of Full-screen image viewer with zoom and pan
 */
...
ImageViewer::ImageViewer(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("UI::ImageViewer", "Creating view...");
    ...
    Logging::debug("UI::ImageViewer", "View created");
}

ImageViewer::~ImageViewer()
{
    Logging::debug("UI::ImageViewer", "Destroying view...");
}

void ImageViewer::activate()
{
    if (m_isActive) {
        Logging::debug("UI::ImageViewer", "View already active");
        return;
    }

    Logging::info("UI::ImageViewer", "Activating view...");
    m_isActive = true;
    ...
    Logging::info("UI::ImageViewer", "View activated");
}

void ImageViewer::deactivate()
{
    if (!m_isActive) {
        return;
    }

    Logging::debug("UI::ImageViewer", "Deactivating view...");
    m_isActive = false;
}

void ImageViewer::refresh()
{
    Logging::debug("UI::ImageViewer", "Refreshing view...");
    m_isEmpty = true;
    m_hasError = false;
    loadContent();
}
...
{
    return QStringLiteral("image_viewer");
}

QIcon ImageViewer::icon() const
{
    return QIcon();
}
...
{
    Logging::debug("UI::ImageViewer", "Showing loading indicator");
    m_isLoading = true;
...
void ImageViewer::showEmptyState(const QString &message)
{
    Logging::debug("UI::ImageViewer",
...
bool ImageViewer::isLoading() const
{
    return m_isLoading;
}

bool ImageViewer::hasError() const
{
    return m_hasError;
}

bool ImageViewer::isEmpty() const
{
    return m_isEmpty;
}
...
{
    Logging::debug("UI::ImageViewer", "Close event received");
    deactivate();
...
{
    Logging::debug("UI::ImageViewer", "Setting up UI...");
    ...
    m_toolbar->setObjectName(QStringLiteral("ImageViewerToolbar"));
    ...
    connect(backAction, &QAction::triggered,
            this, &ImageViewer::backRequested);
    ...
    Logging::debug("UI::ImageViewer", "UI setup complete");
}

void ImageViewer::connectSignals()
{
    Logging::debug("UI::ImageViewer", "Connecting signals...");
    ...
    connect(&Preferences::instance(), &Preferences::settingChanged,
        ...
        if (key == QStringLiteral("theme")) {
            Logging::debug("UI::ImageViewer", "Theme changed, updating...");
            ThemeManager::instance().applyToWidget(this);
        }
    });
    ...
    Logging::debug("UI::ImageViewer", "Signals connected");
}

void ImageViewer::loadContent()
{
    Logging::debug("UI::ImageViewer", "Loading content...");
    showLoading();
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        m_isEmpty = false;
        emit contentLoaded();
        Logging::debug("UI::ImageViewer", "Content loaded successfully");
    });
}
