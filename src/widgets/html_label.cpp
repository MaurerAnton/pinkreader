 ...
 * File: html_label.cpp - Implementation of HTML rendered text label
 */

#include "widgets/html_label.h"
...
HtmlLabel::HtmlLabel(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::HtmlLabel", "Creating widget...");
    ...
    Logging::debug("Widgets::HtmlLabel", "Widget created");
}

HtmlLabel::~HtmlLabel()
{
    Logging::debug("Widgets::HtmlLabel", "Widget destroyed");
...
qreal HtmlLabel::opacity() const
{
    return m_opacity;
}

void HtmlLabel::setOpacity(qreal opacity)
{
...
QColor HtmlLabel::backgroundColor() const
{
    return m_backgroundColor;
}

void HtmlLabel::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int HtmlLabel::animationDuration() const
{
    return m_animationDuration;
}

void HtmlLabel::setAnimationDuration(int ms)
{
...
bool HtmlLabel::isLoading() const
{
    return m_isLoading;
}

void HtmlLabel::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::HtmlLabel", "Showing loading state");
    m_isLoading = true;
    update();
}

void HtmlLabel::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::HtmlLabel", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool HtmlLabel::isInteractionEnabled() const
{
    return m_isEnabled;
}

void HtmlLabel::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void HtmlLabel::setText(const QString &text)
{
...
QString HtmlLabel::text() const
{
    return m_text;
}

void HtmlLabel::setSubtitle(const QString &subtitle)
{
...
void HtmlLabel::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void HtmlLabel::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void HtmlLabel::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void HtmlLabel::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void HtmlLabel::mouseMoveEvent(QMouseEvent *event)
{
...
void HtmlLabel::resizeEvent(QResizeEvent *event)
{
...
bool HtmlLabel::event(QEvent *event)
{
...
void HtmlLabel::setupUi()
{
    Logging::debug("Widgets::HtmlLabel", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("HtmlLabelText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("HtmlLabelSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("HtmlLabelIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("HtmlLabelImage"));
...
void HtmlLabel::connectSignals()
{
    Logging::debug("Widgets::HtmlLabel", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &HtmlLabel::applyTheme);
...
void HtmlLabel::animateOpacity(qreal targetOpacity)
{
