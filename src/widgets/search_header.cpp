 ...
 * File: search_header.cpp - Implementation of Search results header widget
 */

#include "widgets/search_header.h"
...
SearchHeader::SearchHeader(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::SearchHeader", "Creating widget...");
    ...
    Logging::debug("Widgets::SearchHeader", "Widget created");
}

SearchHeader::~SearchHeader()
{
    Logging::debug("Widgets::SearchHeader", "Widget destroyed");
...
qreal SearchHeader::opacity() const
{
    return m_opacity;
}

void SearchHeader::setOpacity(qreal opacity)
{
...
QColor SearchHeader::backgroundColor() const
{
    return m_backgroundColor;
}

void SearchHeader::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int SearchHeader::animationDuration() const
{
    return m_animationDuration;
}

void SearchHeader::setAnimationDuration(int ms)
{
...
bool SearchHeader::isLoading() const
{
    return m_isLoading;
}

void SearchHeader::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::SearchHeader", "Showing loading state");
    m_isLoading = true;
    update();
}

void SearchHeader::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::SearchHeader", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool SearchHeader::isInteractionEnabled() const
{
    return m_isEnabled;
}

void SearchHeader::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void SearchHeader::setText(const QString &text)
{
...
QString SearchHeader::text() const
{
    return m_text;
}

void SearchHeader::setSubtitle(const QString &subtitle)
{
...
void SearchHeader::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void SearchHeader::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void SearchHeader::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void SearchHeader::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void SearchHeader::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void SearchHeader::mouseMoveEvent(QMouseEvent *event)
{
...
void SearchHeader::resizeEvent(QResizeEvent *event)
{
...
bool SearchHeader::event(QEvent *event)
{
...
void SearchHeader::setupUi()
{
    Logging::debug("Widgets::SearchHeader", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("SearchHeaderText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("SearchHeaderSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("SearchHeaderIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("SearchHeaderImage"));
...
void SearchHeader::connectSignals()
{
    Logging::debug("Widgets::SearchHeader", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &SearchHeader::applyTheme);
...
void SearchHeader::animateOpacity(qreal targetOpacity)
{
