 ...
 * File: tab_bar.cpp - Implementation of Tab navigation bar widget
 */
...
TabBar::TabBar(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::TabBar", "Creating widget...");
    ...
    Logging::debug("Widgets::TabBar", "Widget created");
}

TabBar::~TabBar()
{
    Logging::debug("Widgets::TabBar", "Widget destroyed");
...
int TabBar::animationDuration() const
{
    return m_animationDuration;
}

void TabBar::setAnimationDuration(int ms)
{
...
bool TabBar::isLoading() const
{
    return m_isLoading;
}

void TabBar::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::TabBar", "Showing loading state");
    m_isLoading = true;
    update();
}

void TabBar::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::TabBar", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool TabBar::isInteractionEnabled() const
{
    return m_isEnabled;
}

void TabBar::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void TabBar::setText(const QString &text)
{
...
QString TabBar::text() const
{
    return m_text;
}

void TabBar::setSubtitle(const QString &subtitle)
{
...
void TabBar::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void TabBar::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void TabBar::setupUi()
{
    Logging::debug("Widgets::TabBar", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("TabBarText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("TabBarSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("TabBarIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("TabBarImage"));
...
void TabBar::connectSignals()
{
    Logging::debug("Widgets::TabBar", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &TabBar::applyTheme);
...
void TabBar::animateOpacity(qreal targetOpacity)
{
