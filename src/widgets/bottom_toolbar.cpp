 ...
 * File: bottom_toolbar.cpp - Implementation of Bottom action toolbar
 */

#include "widgets/bottom_toolbar.h"
...
BottomToolbar::BottomToolbar(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::BottomToolbar", "Creating widget...");
    ...
    Logging::debug("Widgets::BottomToolbar", "Widget created");
}

BottomToolbar::~BottomToolbar()
{
    Logging::debug("Widgets::BottomToolbar", "Widget destroyed");
...
qreal BottomToolbar::opacity() const
{
    return m_opacity;
}

void BottomToolbar::setOpacity(qreal opacity)
{
...
QColor BottomToolbar::backgroundColor() const
{
    return m_backgroundColor;
}

void BottomToolbar::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int BottomToolbar::animationDuration() const
{
    return m_animationDuration;
}

void BottomToolbar::setAnimationDuration(int ms)
{
...
bool BottomToolbar::isLoading() const
{
    return m_isLoading;
}

void BottomToolbar::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::BottomToolbar", "Showing loading state");
    m_isLoading = true;
    update();
}

void BottomToolbar::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::BottomToolbar", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool BottomToolbar::isInteractionEnabled() const
{
    return m_isEnabled;
}

void BottomToolbar::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void BottomToolbar::setText(const QString &text)
{
...
QString BottomToolbar::text() const
{
    return m_text;
}

void BottomToolbar::setSubtitle(const QString &subtitle)
{
...
void BottomToolbar::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void BottomToolbar::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void BottomToolbar::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void BottomToolbar::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void BottomToolbar::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void BottomToolbar::mouseMoveEvent(QMouseEvent *event)
{
...
bool BottomToolbar::event(QEvent *event)
{
...
void BottomToolbar::setupUi()
{
    Logging::debug("Widgets::BottomToolbar", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("BottomToolbarText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("BottomToolbarSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("BottomToolbarIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("BottomToolbarImage"));
...
void BottomToolbar::connectSignals()
{
    Logging::debug("Widgets::BottomToolbar", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &BottomToolbar::applyTheme);
...
void BottomToolbar::animateOpacity(qreal targetOpacity)
{
