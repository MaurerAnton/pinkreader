 ...
 * File: action_button.cpp - Implementation of Custom action button with icon and label
 */
...
ActionButton::ActionButton(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::ActionButton", "Creating widget...");
    ...
    Logging::debug("Widgets::ActionButton", "Widget created");
}

ActionButton::~ActionButton()
{
    Logging::debug("Widgets::ActionButton", "Widget destroyed");
...
qreal ActionButton::opacity() const
{
    return m_opacity;
}

void ActionButton::setOpacity(qreal opacity)
{
...
QColor ActionButton::backgroundColor() const
{
    return m_backgroundColor;
}

void ActionButton::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int ActionButton::animationDuration() const
{
    return m_animationDuration;
}

void ActionButton::setAnimationDuration(int ms)
{
...
bool ActionButton::isLoading() const
{
    return m_isLoading;
}

void ActionButton::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::ActionButton", "Showing loading state");
    m_isLoading = true;
    update();
}

void ActionButton::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::ActionButton", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool ActionButton::isInteractionEnabled() const
{
    return m_isEnabled;
}

void ActionButton::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
QString ActionButton::text() const
{
    return m_text;
}

void ActionButton::setSubtitle(const QString &subtitle)
{
...
void ActionButton::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void ActionButton::resizeEvent(QResizeEvent *event)
{
...
void ActionButton::setupUi()
{
    Logging::debug("Widgets::ActionButton", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("ActionButtonText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("ActionButtonSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("ActionButtonIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("ActionButtonImage"));
...
void ActionButton::connectSignals()
{
    Logging::debug("Widgets::ActionButton", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &ActionButton::applyTheme);
...
void ActionButton::animateOpacity(qreal targetOpacity)
{
