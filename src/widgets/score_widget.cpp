 ...
 * File: score_widget.cpp - Implementation of Vote score display with animation
 */

#include "widgets/score_widget.h"
...
ScoreWidget::ScoreWidget(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::ScoreWidget", "Creating widget...");
    ...
    Logging::debug("Widgets::ScoreWidget", "Widget created");
}

ScoreWidget::~ScoreWidget()
{
    Logging::debug("Widgets::ScoreWidget", "Widget destroyed");
...
qreal ScoreWidget::opacity() const
{
    return m_opacity;
}

void ScoreWidget::setOpacity(qreal opacity)
{
...
QColor ScoreWidget::backgroundColor() const
{
    return m_backgroundColor;
}

void ScoreWidget::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int ScoreWidget::animationDuration() const
{
    return m_animationDuration;
}

void ScoreWidget::setAnimationDuration(int ms)
{
...
bool ScoreWidget::isLoading() const
{
    return m_isLoading;
}

void ScoreWidget::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::ScoreWidget", "Showing loading state");
    m_isLoading = true;
    update();
}

void ScoreWidget::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::ScoreWidget", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool ScoreWidget::isInteractionEnabled() const
{
    return m_isEnabled;
}

void ScoreWidget::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
QString ScoreWidget::text() const
{
    return m_text;
}

void ScoreWidget::setSubtitle(const QString &subtitle)
{
...
void ScoreWidget::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void ScoreWidget::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
{
    Logging::debug("Widgets::ScoreWidget", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("ScoreWidgetText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("ScoreWidgetSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("ScoreWidgetIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("ScoreWidgetImage"));
...
void ScoreWidget::connectSignals()
{
    Logging::debug("Widgets::ScoreWidget", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &ScoreWidget::applyTheme);
...
void ScoreWidget::animateOpacity(qreal targetOpacity)
{
