 ...
 * File: progress_bar.cpp - Implementation of Determinate progress bar
 */
...
ProgressBar::ProgressBar(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::ProgressBar", "Creating widget...");
    ...
    Logging::debug("Widgets::ProgressBar", "Widget created");
}

ProgressBar::~ProgressBar()
{
    Logging::debug("Widgets::ProgressBar", "Widget destroyed");
...
int ProgressBar::animationDuration() const
{
    return m_animationDuration;
}

void ProgressBar::setAnimationDuration(int ms)
{
...
void ProgressBar::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::ProgressBar", "Showing loading state");
    m_isLoading = true;
    update();
}

void ProgressBar::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::ProgressBar", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool ProgressBar::isInteractionEnabled() const
{
    return m_isEnabled;
}

void ProgressBar::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void ProgressBar::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void ProgressBar::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
{
    Logging::debug("Widgets::ProgressBar", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("ProgressBarText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("ProgressBarSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("ProgressBarIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("ProgressBarImage"));
...
void ProgressBar::connectSignals()
{
    Logging::debug("Widgets::ProgressBar", "Connecting signals...");
...
void ProgressBar::animateOpacity(qreal targetOpacity)
{
