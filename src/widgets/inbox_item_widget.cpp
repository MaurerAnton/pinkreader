 ...
 * File: inbox_item_widget.cpp - Implementation of Inbox/message item widget
 */

#include "widgets/inbox_item_widget.h"
...
InboxItemWidget::InboxItemWidget(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::InboxItemWidget", "Creating widget...");
    ...
    Logging::debug("Widgets::InboxItemWidget", "Widget created");
}

InboxItemWidget::~InboxItemWidget()
{
    Logging::debug("Widgets::InboxItemWidget", "Widget destroyed");
...
qreal InboxItemWidget::opacity() const
{
    return m_opacity;
}

void InboxItemWidget::setOpacity(qreal opacity)
{
...
QColor InboxItemWidget::backgroundColor() const
{
    return m_backgroundColor;
}

void InboxItemWidget::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int InboxItemWidget::animationDuration() const
{
    return m_animationDuration;
}

void InboxItemWidget::setAnimationDuration(int ms)
{
...
bool InboxItemWidget::isLoading() const
{
    return m_isLoading;
}

void InboxItemWidget::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::InboxItemWidget", "Showing loading state");
    m_isLoading = true;
    update();
}

void InboxItemWidget::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::InboxItemWidget", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool InboxItemWidget::isInteractionEnabled() const
{
    return m_isEnabled;
}

void InboxItemWidget::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void InboxItemWidget::setText(const QString &text)
{
...
QString InboxItemWidget::text() const
{
    return m_text;
}

void InboxItemWidget::setSubtitle(const QString &subtitle)
{
...
void InboxItemWidget::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void InboxItemWidget::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void InboxItemWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void InboxItemWidget::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void InboxItemWidget::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void InboxItemWidget::mouseMoveEvent(QMouseEvent *event)
{
...
void InboxItemWidget::resizeEvent(QResizeEvent *event)
{
...
bool InboxItemWidget::event(QEvent *event)
{
...
void InboxItemWidget::setupUi()
{
    Logging::debug("Widgets::InboxItemWidget", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("InboxItemWidgetText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("InboxItemWidgetSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("InboxItemWidgetIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("InboxItemWidgetImage"));
...
void InboxItemWidget::connectSignals()
{
    Logging::debug("Widgets::InboxItemWidget", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &InboxItemWidget::applyTheme);
...
void InboxItemWidget::applyTheme()
{
...
void InboxItemWidget::animateOpacity(qreal targetOpacity)
{
