 ...
 * File: video_player.cpp - Implementation of Video playback widget with controls
 */

#include "widgets/video_player.h"
...
VideoPlayer::VideoPlayer(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::VideoPlayer", "Creating widget...");
    ...
    Logging::debug("Widgets::VideoPlayer", "Widget created");
}

VideoPlayer::~VideoPlayer()
{
    Logging::debug("Widgets::VideoPlayer", "Widget destroyed");
...
qreal VideoPlayer::opacity() const
{
    return m_opacity;
}

void VideoPlayer::setOpacity(qreal opacity)
{
...
QColor VideoPlayer::backgroundColor() const
{
    return m_backgroundColor;
}

void VideoPlayer::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int VideoPlayer::animationDuration() const
{
    return m_animationDuration;
}

void VideoPlayer::setAnimationDuration(int ms)
{
...
void VideoPlayer::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::VideoPlayer", "Showing loading state");
    m_isLoading = true;
    update();
}

void VideoPlayer::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::VideoPlayer", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool VideoPlayer::isInteractionEnabled() const
{
    return m_isEnabled;
}

void VideoPlayer::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void VideoPlayer::setText(const QString &text)
{
...
QString VideoPlayer::text() const
{
    return m_text;
}

void VideoPlayer::setSubtitle(const QString &subtitle)
{
...
void VideoPlayer::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void VideoPlayer::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
{
    Logging::debug("Widgets::VideoPlayer", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("VideoPlayerText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("VideoPlayerSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("VideoPlayerIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("VideoPlayerImage"));
...
{
    Logging::debug("Widgets::VideoPlayer", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &VideoPlayer::applyTheme);
...
void VideoPlayer::animateOpacity(qreal targetOpacity)
{
