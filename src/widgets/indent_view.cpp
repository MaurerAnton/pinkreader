 ...
 * File: indent_view.cpp - Implementation of Comment thread indentation widget
 */

#include "widgets/indent_view.h"
...
IndentView::IndentView(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::IndentView", "Creating widget...");
    ...
    Logging::debug("Widgets::IndentView", "Widget created");
}

IndentView::~IndentView()
{
    Logging::debug("Widgets::IndentView", "Widget destroyed");
...
qreal IndentView::opacity() const
{
    return m_opacity;
}

void IndentView::setOpacity(qreal opacity)
{
...
QColor IndentView::backgroundColor() const
{
    return m_backgroundColor;
}

void IndentView::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int IndentView::animationDuration() const
{
    return m_animationDuration;
}

void IndentView::setAnimationDuration(int ms)
{
...
bool IndentView::isLoading() const
{
    return m_isLoading;
}

void IndentView::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::IndentView", "Showing loading state");
    m_isLoading = true;
    update();
}

void IndentView::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::IndentView", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool IndentView::isInteractionEnabled() const
{
    return m_isEnabled;
}

void IndentView::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void IndentView::setText(const QString &text)
{
...
QString IndentView::text() const
{
    return m_text;
}

void IndentView::setSubtitle(const QString &subtitle)
{
...
void IndentView::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void IndentView::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void IndentView::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void IndentView::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void IndentView::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void IndentView::mouseMoveEvent(QMouseEvent *event)
{
...
void IndentView::resizeEvent(QResizeEvent *event)
{
...
bool IndentView::event(QEvent *event)
{
...
void IndentView::setupUi()
{
    Logging::debug("Widgets::IndentView", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("IndentViewText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("IndentViewSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("IndentViewIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("IndentViewImage"));
...
void IndentView::connectSignals()
{
    Logging::debug("Widgets::IndentView", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &IndentView::applyTheme);
...
void IndentView::animateOpacity(qreal targetOpacity)
{
