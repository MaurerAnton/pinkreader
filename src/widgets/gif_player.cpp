 ...
 * File: gif_player.cpp - Implementation of GIF animation playback widget
 */

#include "widgets/gif_player.h"
...
GifPlayer::GifPlayer(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::GifPlayer", "Creating widget...");
    ...
    Logging::debug("Widgets::GifPlayer", "Widget created");
}

GifPlayer::~GifPlayer()
{
    Logging::debug("Widgets::GifPlayer", "Widget destroyed");
...
qreal GifPlayer::opacity() const
{
    return m_opacity;
}

void GifPlayer::setOpacity(qreal opacity)
{
...
QColor GifPlayer::backgroundColor() const
{
    return m_backgroundColor;
}

void GifPlayer::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int GifPlayer::animationDuration() const
{
    return m_animationDuration;
}

void GifPlayer::setAnimationDuration(int ms)
{
...
bool GifPlayer::isLoading() const
{
    return m_isLoading;
}

void GifPlayer::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::GifPlayer", "Showing loading state");
    m_isLoading = true;
    update();
}

void GifPlayer::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::GifPlayer", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool GifPlayer::isInteractionEnabled() const
{
    return m_isEnabled;
}

void GifPlayer::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void GifPlayer::setText(const QString &text)
{
...
QString GifPlayer::text() const
{
    return m_text;
}

void GifPlayer::setSubtitle(const QString &subtitle)
{
...
void GifPlayer::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void GifPlayer::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void GifPlayer::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void GifPlayer::mouseMoveEvent(QMouseEvent *event)
{
...
void GifPlayer::resizeEvent(QResizeEvent *event)
{
...
void GifPlayer::setupUi()
{
    Logging::debug("Widgets::GifPlayer", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("GifPlayerText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("GifPlayerSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("GifPlayerIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("GifPlayerImage"));
...
void GifPlayer::connectSignals()
{
    Logging::debug("Widgets::GifPlayer", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &GifPlayer::applyTheme);
...
void GifPlayer::animateOpacity(qreal targetOpacity)
{
