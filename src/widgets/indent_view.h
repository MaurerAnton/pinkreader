 ...
 * File: indent_view.h - Comment thread indentation widget
 */
...
/**
 * @brief Comment thread indentation widget
 *
 * A custom UI widget providing comment thread indentation widget.
 * Designed for optimal performance on mobile devices with
 * smooth animations, gesture support, and accessibility features.
 */
class IndentView : public QWidget
{
    Q_OBJECT
...
public:
    /**
     * @brief Construct the IndentView widget
     * @param parent Parent widget (nullptr for top-level)
     */
    explicit IndentView(QWidget *parent = nullptr);
