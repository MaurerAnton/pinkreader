 ...
 * File: load_more_button.cpp - Implementation of Load more content button
 */

#include "widgets/load_more_button.h"
...
LoadMoreButton::LoadMoreButton(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::LoadMoreButton", "Creating widget...");
    ...
    Logging::debug("Widgets::LoadMoreButton", "Widget created");
}

LoadMoreButton::~LoadMoreButton()
{
    Logging::debug("Widgets::LoadMoreButton", "Widget destroyed");
...
qreal LoadMoreButton::opacity() const
{
    return m_opacity;
}

void LoadMoreButton::setOpacity(qreal opacity)
{
...
QColor LoadMoreButton::backgroundColor() const
{
    return m_backgroundColor;
}

void LoadMoreButton::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int LoadMoreButton::animationDuration() const
{
    return m_animationDuration;
}

void LoadMoreButton::setAnimationDuration(int ms)
{
...
bool LoadMoreButton::isLoading() const
{
    return m_isLoading;
}

void LoadMoreButton::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::LoadMoreButton", "Showing loading state");
    m_isLoading = true;
    update();
}

void LoadMoreButton::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::LoadMoreButton", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool LoadMoreButton::isInteractionEnabled() const
{
    return m_isEnabled;
}

void LoadMoreButton::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void LoadMoreButton::setText(const QString &text)
{
...
QString LoadMoreButton::text() const
{
    return m_text;
}

void LoadMoreButton::setSubtitle(const QString &subtitle)
{
...
void LoadMoreButton::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void LoadMoreButton::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void LoadMoreButton::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LoadMoreButton::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LoadMoreButton::mouseMoveEvent(QMouseEvent *event)
{
...
void LoadMoreButton::resizeEvent(QResizeEvent *event)
{
...
bool LoadMoreButton::event(QEvent *event)
{
...
void LoadMoreButton::setupUi()
{
    Logging::debug("Widgets::LoadMoreButton", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("LoadMoreButtonText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("LoadMoreButtonSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("LoadMoreButtonIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("LoadMoreButtonImage"));
...
void LoadMoreButton::connectSignals()
{
    Logging::debug("Widgets::LoadMoreButton", "Connecting signals...");
...
void LoadMoreButton::applyTheme()
{
...
void LoadMoreButton::animateOpacity(qreal targetOpacity)
{
