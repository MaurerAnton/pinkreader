 ...
 * File: inbox_item_widget.h - Inbox/message item widget
 */
...
/**
 * @brief Inbox/message item widget
 *
 * A custom UI widget providing inbox/message item widget.
 * Designed for optimal performance on mobile devices with
 * smooth animations, gesture support, and accessibility features.
 */
class InboxItemWidget : public QWidget
{
    Q_OBJECT
...
public:
    /**
     * @brief Construct the InboxItemWidget widget
     * @param parent Parent widget (nullptr for top-level)
     */
    explicit InboxItemWidget(QWidget *parent = nullptr);

    /**
     * @brief Destructor
     */
    ~InboxItemWidget() override;
