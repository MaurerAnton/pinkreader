 ...
 * File: comment_widget.cpp - Implementation of Comment rendering widget
 */

#include "widgets/comment_widget.h"
...
CommentWidget::CommentWidget(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::CommentWidget", "Creating widget...");
    ...
    Logging::debug("Widgets::CommentWidget", "Widget created");
}

CommentWidget::~CommentWidget()
{
    Logging::debug("Widgets::CommentWidget", "Widget destroyed");
...
qreal CommentWidget::opacity() const
{
    return m_opacity;
}

void CommentWidget::setOpacity(qreal opacity)
{
...
QColor CommentWidget::backgroundColor() const
{
    return m_backgroundColor;
}

void CommentWidget::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int CommentWidget::animationDuration() const
{
    return m_animationDuration;
}

void CommentWidget::setAnimationDuration(int ms)
{
...
bool CommentWidget::isLoading() const
{
    return m_isLoading;
}

void CommentWidget::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::CommentWidget", "Showing loading state");
    m_isLoading = true;
    update();
}

void CommentWidget::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::CommentWidget", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool CommentWidget::isInteractionEnabled() const
{
    return m_isEnabled;
}

void CommentWidget::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void CommentWidget::setText(const QString &text)
{
...
void CommentWidget::setSubtitle(const QString &subtitle)
{
...
void CommentWidget::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void CommentWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void CommentWidget::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void CommentWidget::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void CommentWidget::mouseMoveEvent(QMouseEvent *event)
{
...
void CommentWidget::resizeEvent(QResizeEvent *event)
{
...
bool CommentWidget::event(QEvent *event)
{
...
{
    Logging::debug("Widgets::CommentWidget", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("CommentWidgetText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("CommentWidgetSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("CommentWidgetIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("CommentWidgetImage"));
...
void CommentWidget::connectSignals()
{
    Logging::debug("Widgets::CommentWidget", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &CommentWidget::applyTheme);
...
void CommentWidget::animateOpacity(qreal targetOpacity)
{
