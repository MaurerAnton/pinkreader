 ...
 * File: loading_spinner.cpp - Implementation of Loading/progress spinner widget
 */

#include "widgets/loading_spinner.h"
...
LoadingSpinner::LoadingSpinner(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::LoadingSpinner", "Creating widget...");
    ...
    Logging::debug("Widgets::LoadingSpinner", "Widget created");
}

LoadingSpinner::~LoadingSpinner()
{
    Logging::debug("Widgets::LoadingSpinner", "Widget destroyed");
...
qreal LoadingSpinner::opacity() const
{
    return m_opacity;
}

void LoadingSpinner::setOpacity(qreal opacity)
{
...
QColor LoadingSpinner::backgroundColor() const
{
    return m_backgroundColor;
}

void LoadingSpinner::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int LoadingSpinner::animationDuration() const
{
    return m_animationDuration;
}

void LoadingSpinner::setAnimationDuration(int ms)
{
...
bool LoadingSpinner::isLoading() const
{
    return m_isLoading;
}

void LoadingSpinner::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::LoadingSpinner", "Showing loading state");
    m_isLoading = true;
    update();
}

void LoadingSpinner::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::LoadingSpinner", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool LoadingSpinner::isInteractionEnabled() const
{
    return m_isEnabled;
}

void LoadingSpinner::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void LoadingSpinner::setText(const QString &text)
{
...
QString LoadingSpinner::text() const
{
    return m_text;
}

void LoadingSpinner::setSubtitle(const QString &subtitle)
{
...
void LoadingSpinner::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void LoadingSpinner::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void LoadingSpinner::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LoadingSpinner::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LoadingSpinner::mouseMoveEvent(QMouseEvent *event)
{
...
void LoadingSpinner::resizeEvent(QResizeEvent *event)
{
...
bool LoadingSpinner::event(QEvent *event)
{
...
void LoadingSpinner::setupUi()
{
    Logging::debug("Widgets::LoadingSpinner", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("LoadingSpinnerText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("LoadingSpinnerSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("LoadingSpinnerIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("LoadingSpinnerImage"));
...
void LoadingSpinner::connectSignals()
{
    Logging::debug("Widgets::LoadingSpinner", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &LoadingSpinner::applyTheme);
...
void LoadingSpinner::applyTheme()
{
...
void LoadingSpinner::animateOpacity(qreal targetOpacity)
{
