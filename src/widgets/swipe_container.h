 ...
 * File: swipe_container.h - Swipe gesture container with actions
 */
...
/**
 * @brief Swipe gesture container with actions
 *
 * A custom UI widget providing swipe gesture container with actions.
 * Designed for optimal performance on mobile devices with
 * smooth animations, gesture support, and accessibility features.
 */
class SwipeContainer : public QWidget
{
    Q_OBJECT
...
public:
    /**
     * @brief Construct the SwipeContainer widget
     * @param parent Parent widget (nullptr for top-level)
     */
    explicit SwipeContainer(QWidget *parent = nullptr);

    /**
     * @brief Destructor
     */
    ~SwipeContainer() override;
