 ...
 * File: markdown_label.cpp - Implementation of Markdown rendered text label
 */

#include "widgets/markdown_label.h"
...
MarkdownLabel::MarkdownLabel(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::MarkdownLabel", "Creating widget...");
    ...
    Logging::debug("Widgets::MarkdownLabel", "Widget created");
}

MarkdownLabel::~MarkdownLabel()
{
    Logging::debug("Widgets::MarkdownLabel", "Widget destroyed");
...
qreal MarkdownLabel::opacity() const
{
    return m_opacity;
}

void MarkdownLabel::setOpacity(qreal opacity)
{
...
QColor MarkdownLabel::backgroundColor() const
{
    return m_backgroundColor;
}

void MarkdownLabel::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int MarkdownLabel::animationDuration() const
{
    return m_animationDuration;
}

void MarkdownLabel::setAnimationDuration(int ms)
{
...
bool MarkdownLabel::isLoading() const
{
    return m_isLoading;
}

void MarkdownLabel::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::MarkdownLabel", "Showing loading state");
    m_isLoading = true;
    update();
}

void MarkdownLabel::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::MarkdownLabel", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool MarkdownLabel::isInteractionEnabled() const
{
    return m_isEnabled;
}

void MarkdownLabel::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
QString MarkdownLabel::text() const
{
    return m_text;
}

void MarkdownLabel::setSubtitle(const QString &subtitle)
{
...
void MarkdownLabel::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void MarkdownLabel::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
bool MarkdownLabel::event(QEvent *event)
{
...
void MarkdownLabel::setupUi()
{
    Logging::debug("Widgets::MarkdownLabel", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("MarkdownLabelText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("MarkdownLabelSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("MarkdownLabelIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("MarkdownLabelImage"));
...
void MarkdownLabel::connectSignals()
{
    Logging::debug("Widgets::MarkdownLabel", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &MarkdownLabel::applyTheme);
...
void MarkdownLabel::animateOpacity(qreal targetOpacity)
{
