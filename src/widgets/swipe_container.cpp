 ...
 * File: swipe_container.cpp - Implementation of Swipe gesture container with actions
 */

#include "widgets/swipe_container.h"
...
SwipeContainer::SwipeContainer(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::SwipeContainer", "Creating widget...");
    ...
    Logging::debug("Widgets::SwipeContainer", "Widget created");
}

SwipeContainer::~SwipeContainer()
{
    Logging::debug("Widgets::SwipeContainer", "Widget destroyed");
...
qreal SwipeContainer::opacity() const
{
    return m_opacity;
}

void SwipeContainer::setOpacity(qreal opacity)
{
...
QColor SwipeContainer::backgroundColor() const
{
    return m_backgroundColor;
}

void SwipeContainer::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int SwipeContainer::animationDuration() const
{
    return m_animationDuration;
}

void SwipeContainer::setAnimationDuration(int ms)
{
...
bool SwipeContainer::isLoading() const
{
    return m_isLoading;
}

void SwipeContainer::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::SwipeContainer", "Showing loading state");
    m_isLoading = true;
    update();
}

void SwipeContainer::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::SwipeContainer", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool SwipeContainer::isInteractionEnabled() const
{
    return m_isEnabled;
}

void SwipeContainer::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void SwipeContainer::setText(const QString &text)
{
...
QString SwipeContainer::text() const
{
    return m_text;
}

void SwipeContainer::setSubtitle(const QString &subtitle)
{
...
void SwipeContainer::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void SwipeContainer::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void SwipeContainer::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void SwipeContainer::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void SwipeContainer::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void SwipeContainer::mouseMoveEvent(QMouseEvent *event)
{
...
void SwipeContainer::resizeEvent(QResizeEvent *event)
{
...
bool SwipeContainer::event(QEvent *event)
{
...
void SwipeContainer::setupUi()
{
    Logging::debug("Widgets::SwipeContainer", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("SwipeContainerText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("SwipeContainerSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("SwipeContainerIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("SwipeContainerImage"));
...
void SwipeContainer::connectSignals()
{
    Logging::debug("Widgets::SwipeContainer", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &SwipeContainer::applyTheme);
...
void SwipeContainer::applyTheme()
{
...
void SwipeContainer::animateOpacity(qreal targetOpacity)
{
