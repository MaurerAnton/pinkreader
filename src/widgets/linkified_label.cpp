 ...
 * File: linkified_label.cpp - Implementation of Auto-link detection text label
 */

#include "widgets/linkified_label.h"
...
LinkifiedLabel::LinkifiedLabel(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::LinkifiedLabel", "Creating widget...");
    ...
    Logging::debug("Widgets::LinkifiedLabel", "Widget created");
}

LinkifiedLabel::~LinkifiedLabel()
{
    Logging::debug("Widgets::LinkifiedLabel", "Widget destroyed");
...
qreal LinkifiedLabel::opacity() const
{
    return m_opacity;
}

void LinkifiedLabel::setOpacity(qreal opacity)
{
...
QColor LinkifiedLabel::backgroundColor() const
{
    return m_backgroundColor;
}

void LinkifiedLabel::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int LinkifiedLabel::animationDuration() const
{
    return m_animationDuration;
}

void LinkifiedLabel::setAnimationDuration(int ms)
{
...
bool LinkifiedLabel::isLoading() const
{
    return m_isLoading;
}

void LinkifiedLabel::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::LinkifiedLabel", "Showing loading state");
    m_isLoading = true;
    update();
}

void LinkifiedLabel::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::LinkifiedLabel", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool LinkifiedLabel::isInteractionEnabled() const
{
    return m_isEnabled;
}

void LinkifiedLabel::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void LinkifiedLabel::setText(const QString &text)
{
...
QString LinkifiedLabel::text() const
{
    return m_text;
}

void LinkifiedLabel::setSubtitle(const QString &subtitle)
{
...
void LinkifiedLabel::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void LinkifiedLabel::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void LinkifiedLabel::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void LinkifiedLabel::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LinkifiedLabel::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void LinkifiedLabel::mouseMoveEvent(QMouseEvent *event)
{
...
void LinkifiedLabel::resizeEvent(QResizeEvent *event)
{
...
bool LinkifiedLabel::event(QEvent *event)
{
...
void LinkifiedLabel::setupUi()
{
    Logging::debug("Widgets::LinkifiedLabel", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("LinkifiedLabelText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("LinkifiedLabelSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("LinkifiedLabelIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("LinkifiedLabelImage"));
...
void LinkifiedLabel::connectSignals()
{
    Logging::debug("Widgets::LinkifiedLabel", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &LinkifiedLabel::applyTheme);
...
void LinkifiedLabel::applyTheme()
{
...
void LinkifiedLabel::animateOpacity(qreal targetOpacity)
{
