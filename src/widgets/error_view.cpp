 ...
 * File: error_view.cpp - Implementation of Error state display widget
 */

#include "widgets/error_view.h"
...
ErrorView::ErrorView(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::ErrorView", "Creating widget...");
    ...
    Logging::debug("Widgets::ErrorView", "Widget created");
}

ErrorView::~ErrorView()
{
    Logging::debug("Widgets::ErrorView", "Widget destroyed");
...
qreal ErrorView::opacity() const
{
    return m_opacity;
}

void ErrorView::setOpacity(qreal opacity)
{
...
QColor ErrorView::backgroundColor() const
{
    return m_backgroundColor;
}

void ErrorView::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int ErrorView::animationDuration() const
{
    return m_animationDuration;
}

void ErrorView::setAnimationDuration(int ms)
{
...
bool ErrorView::isLoading() const
{
    return m_isLoading;
}

void ErrorView::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::ErrorView", "Showing loading state");
    m_isLoading = true;
    update();
}

void ErrorView::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::ErrorView", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool ErrorView::isInteractionEnabled() const
{
    return m_isEnabled;
}

void ErrorView::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void ErrorView::setText(const QString &text)
{
...
QString ErrorView::text() const
{
    return m_text;
}

void ErrorView::setSubtitle(const QString &subtitle)
{
...
void ErrorView::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void ErrorView::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void ErrorView::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void ErrorView::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void ErrorView::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void ErrorView::mouseMoveEvent(QMouseEvent *event)
{
...
void ErrorView::resizeEvent(QResizeEvent *event)
{
...
bool ErrorView::event(QEvent *event)
{
...
void ErrorView::setupUi()
{
    Logging::debug("Widgets::ErrorView", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("ErrorViewText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("ErrorViewSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("ErrorViewIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("ErrorViewImage"));
...
void ErrorView::connectSignals()
{
    Logging::debug("Widgets::ErrorView", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &ErrorView::applyTheme);
...
void ErrorView::animateOpacity(qreal targetOpacity)
{
