 ...
 * File: post_card.cpp - Implementation of Post card widget for post listings
 */

#include "widgets/post_card.h"
...
PostCard::PostCard(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::PostCard", "Creating widget...");
    ...
    Logging::debug("Widgets::PostCard", "Widget created");
}

PostCard::~PostCard()
{
    Logging::debug("Widgets::PostCard", "Widget destroyed");
...
qreal PostCard::opacity() const
{
    return m_opacity;
}

void PostCard::setOpacity(qreal opacity)
{
...
QColor PostCard::backgroundColor() const
{
    return m_backgroundColor;
}

void PostCard::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int PostCard::animationDuration() const
{
    return m_animationDuration;
}

void PostCard::setAnimationDuration(int ms)
{
...
bool PostCard::isLoading() const
{
    return m_isLoading;
}

void PostCard::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::PostCard", "Showing loading state");
    m_isLoading = true;
    update();
}

void PostCard::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::PostCard", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool PostCard::isInteractionEnabled() const
{
    return m_isEnabled;
}

void PostCard::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void PostCard::setText(const QString &text)
{
...
QString PostCard::text() const
{
    return m_text;
}

void PostCard::setSubtitle(const QString &subtitle)
{
...
void PostCard::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void PostCard::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void PostCard::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void PostCard::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void PostCard::mouseMoveEvent(QMouseEvent *event)
{
...
void PostCard::resizeEvent(QResizeEvent *event)
{
...
bool PostCard::event(QEvent *event)
{
...
{
    Logging::debug("Widgets::PostCard", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("PostCardText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("PostCardSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("PostCardIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("PostCardImage"));
...
void PostCard::connectSignals()
{
    Logging::debug("Widgets::PostCard", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &PostCard::applyTheme);
...
void PostCard::animateOpacity(qreal targetOpacity)
{
