 ...
 * File: announcement_banner.cpp - Implementation of Developer announcement banner
 */

#include "widgets/announcement_banner.h"
...
AnnouncementBanner::AnnouncementBanner(QWidget *parent)
    : QWidget(parent)
{
    Logging::debug("Widgets::AnnouncementBanner", "Creating widget...");
    ...
    Logging::debug("Widgets::AnnouncementBanner", "Widget created");
}

AnnouncementBanner::~AnnouncementBanner()
{
    Logging::debug("Widgets::AnnouncementBanner", "Widget destroyed");
...
qreal AnnouncementBanner::opacity() const
{
    return m_opacity;
}

void AnnouncementBanner::setOpacity(qreal opacity)
{
...
QColor AnnouncementBanner::backgroundColor() const
{
    return m_backgroundColor;
}

void AnnouncementBanner::setBackgroundColor(const QColor &color)
{
    m_backgroundColor = color;
    update();
}

int AnnouncementBanner::animationDuration() const
{
    return m_animationDuration;
}

void AnnouncementBanner::setAnimationDuration(int ms)
{
...
bool AnnouncementBanner::isLoading() const
{
    return m_isLoading;
}

void AnnouncementBanner::showLoading()
{
    if (m_isLoading) return;

    Logging::debug("Widgets::AnnouncementBanner", "Showing loading state");
    m_isLoading = true;
    update();
}

void AnnouncementBanner::hideLoading()
{
    if (!m_isLoading) return;

    Logging::debug("Widgets::AnnouncementBanner", "Hiding loading state");
    m_isLoading = false;
    update();
}

bool AnnouncementBanner::isInteractionEnabled() const
{
    return m_isEnabled;
}

void AnnouncementBanner::setInteractionEnabled(bool enabled)
{
    m_isEnabled = enabled;
    setEnabled(enabled);
    update();
...
void AnnouncementBanner::setText(const QString &text)
{
...
QString AnnouncementBanner::text() const
{
    return m_text;
}

void AnnouncementBanner::setSubtitle(const QString &subtitle)
{
...
void AnnouncementBanner::setIcon(const QIcon &icon)
{
    m_icon = icon;
    if (m_iconLabel) {
...
void AnnouncementBanner::setPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
...
void AnnouncementBanner::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void AnnouncementBanner::mousePressEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void AnnouncementBanner::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_isEnabled) {
        event->ignore();
        return;
...
void AnnouncementBanner::mouseMoveEvent(QMouseEvent *event)
{
...
void AnnouncementBanner::resizeEvent(QResizeEvent *event)
{
...
bool AnnouncementBanner::event(QEvent *event)
{
...
void AnnouncementBanner::setupUi()
{
    Logging::debug("Widgets::AnnouncementBanner", "Setting up UI...");
    ...
    m_textLabel->setObjectName(QStringLiteral("AnnouncementBannerText"));
    ...
    m_subtitleLabel->setObjectName(QStringLiteral("AnnouncementBannerSubtitle"));
    ...
    m_iconLabel->setObjectName(QStringLiteral("AnnouncementBannerIcon"));
    m_iconLabel->setFixedSize(24, 24);
    m_iconLabel->setAlignment(Qt::AlignCenter);

    // Image label
    m_imageLabel = new QLabel(this);
    m_imageLabel->setObjectName(QStringLiteral("AnnouncementBannerImage"));
...
void AnnouncementBanner::connectSignals()
{
    Logging::debug("Widgets::AnnouncementBanner", "Connecting signals...");

    // Theme change handling
    connect(&ThemeManager::instance(), &ThemeManager::themeApplied,
            this, &AnnouncementBanner::applyTheme);
...
void AnnouncementBanner::applyTheme()
{
...
void AnnouncementBanner::animateOpacity(qreal targetOpacity)
{
