 ...
 * File: flair_label.h - User/post flair display widget
 */
...
/**
 * @brief User/post flair display widget
 *
 * A custom UI widget providing user/post flair display widget.
 * Designed for optimal performance on mobile devices with
 * smooth animations, gesture support, and accessibility features.
 */
class FlairLabel : public QWidget
{
    Q_OBJECT
...
public:
    /**
     * @brief Construct the FlairLabel widget
     * @param parent Parent widget (nullptr for top-level)
     */
    explicit FlairLabel(QWidget *parent = nullptr);
