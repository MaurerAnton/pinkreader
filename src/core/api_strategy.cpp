...
StrategyChain::StrategyChain(QObject* parent)
    : QObject(parent) {}
...
void StrategyChain::fetchFeed(const FeedRequest& request, PostCallback callback) {
    ...
    m_strategies[0]->fetchFeed(request, [this, request, callback, idx = 0](bool ok, auto posts, auto after, auto err) mutable {
        if (ok) {
...
void StrategyChain::fetchComments(const CommentRequest& request, CommentCallback callback) {
    ...
    m_strategies[0]->fetchComments(request, [this, request, callback, idx = 0](bool ok, auto comments, auto err) mutable {
        if (ok) {
...
void StrategyChain::fetchSubreddit(const QString& subreddit, SubredditCallback callback) {
    ...
    m_strategies[0]->fetchSubreddit(subreddit, [this, subreddit, callback, idx = 0](bool ok, auto sr, auto err) mutable {
        if (ok) {
