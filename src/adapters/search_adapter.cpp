/*
 * PinkReader - GPLv3
 * File: search_adapter.cpp - Implementation of Search results adapter
 */

#include "adapters/search_adapter.h"
#include "utils/logging.h"
#include <algorithm>

namespace PinkReader {

SearchAdapter::SearchAdapter(QObject *parent) : QAbstractListModel(parent) {}
SearchAdapter::~SearchAdapter() = default;

int SearchAdapter::rowCount(const QModelIndex &parent) const
{
...
QVariant SearchAdapter::data(const QModelIndex &index, int role) const
{
...
QHash<int, QByteArray> SearchAdapter::roleNames() const
{
...
void SearchAdapter::setItems(const QJsonArray &items)
{
    beginResetModel();
...
void SearchAdapter::addItems(const QJsonArray &items)
{
...
int SearchAdapter::itemCount() const
{
    QMutexLocker l(&m_mutex);
    return m_items.size();
}

bool SearchAdapter::isEmpty() const
{
    QMutexLocker l(&m_mutex);
    return m_items.isEmpty();
}

QVariant SearchAdapter::getData(const QJsonObject &item, int role) const
{
    switch (role) {
