/*
 * PinkReader - GPLv3
 * File: draft_adapter.cpp - Implementation of Drafts list adapter
 */

#include "adapters/draft_adapter.h"
#include "utils/logging.h"
#include <algorithm>

namespace PinkReader {

DraftAdapter::DraftAdapter(QObject *parent) : QAbstractListModel(parent) {}
DraftAdapter::~DraftAdapter() = default;

int DraftAdapter::rowCount(const QModelIndex &parent) const
{
...
QVariant DraftAdapter::data(const QModelIndex &index, int role) const
{
...
QHash<int, QByteArray> DraftAdapter::roleNames() const
{
...
void DraftAdapter::setItems(const QJsonArray &items)
{
    beginResetModel();
...
void DraftAdapter::addItems(const QJsonArray &items)
{
...
int DraftAdapter::itemCount() const
{
    QMutexLocker l(&m_mutex);
    return m_items.size();
}

bool DraftAdapter::isEmpty() const
{
    QMutexLocker l(&m_mutex);
    return m_items.isEmpty();
}

QVariant DraftAdapter::getData(const QJsonObject &item, int role) const
{
    switch (role) {
