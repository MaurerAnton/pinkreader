 ...
 * GPLv3 License
 * File: cache_request.cpp - Implementation of Cache request with callbacks
 */
...
CacheRequest::CacheRequest(QObject *parent)
    : QObject(parent)
{
}
...
bool CacheRequest::initialize(const QString &basePath)
{
    if (m_initialized) {
        Logging::debug("Cache::CacheRequest", "Already initialized");
        return true;
    }

    Logging::info("Cache::CacheRequest",
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(QStringLiteral("."))) {
            Logging::error("Cache::CacheRequest",
...
bool CacheRequest::put(const QString &key, const QByteArray &data,
...
QByteArray CacheRequest::get(const QString &key) const
{
    if (!m_initialized) {
        Logging::error("Cache::CacheRequest",
...
bool CacheRequest::remove(const QString &key)
{
    if (!m_initialized)
        return false;
...
bool CacheRequest::contains(const QString &key) const
{
    if (!m_initialized)
        return false;
...
void CacheRequest::clear()
{
    ...
    emit cacheCleared();
    Logging::info("Cache::CacheRequest", "Cache cleared");
}

qint64 CacheRequest::totalSize() const
{
    if (!m_initialized)
        return 0;
...
qint64 CacheRequest::entryCount() const
{
    if (!m_initialized)
        return 0;
...
void CacheRequest::setMaxSize(qint64 maxSizeBytes)
{
    m_maxSize = qMax(static_cast<qint64>(1024 * 1024), maxSizeBytes);
    Logging::debug("Cache::CacheRequest",
        QString("Max size set to: %1 MB").arg(m_maxSize / (1024 * 1024)));
}

qint64 CacheRequest::maxSize() const
{
    return m_maxSize;
}

void CacheRequest::prune()
{
    if (!m_initialized)
        return;
...
void CacheRequest::close()
{
    if (!m_initialized)
        return;

    Logging::info("Cache::CacheRequest", "Closing cache...");
    flush();
    m_initialized = false;
    Logging::info("Cache::CacheRequest", "Cache closed");
}

// Private helpers
QString CacheRequest::entryPath(const QString &key) const
{
    return QDir(m_basePath).absoluteFilePath(key);
}

QVector<CacheRequest::CacheEntry> CacheRequest::loadIndex()
{
    return QVector<CacheEntry>();
}

void CacheRequest::saveIndex(const QVector<CacheEntry> &entries)
{
    Q_UNUSED(entries)
}

void CacheRequest::removeEntryFiles(const CacheEntry &entry)
{
