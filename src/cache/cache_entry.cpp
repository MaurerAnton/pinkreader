 ...
 * GPLv3 License
 * File: cache_entry.cpp - Implementation of Cache entry metadata and data model
 */
...
CacheEntry::CacheEntry(QObject *parent)
    : QObject(parent)
{
}
...
bool CacheEntry::initialize(const QString &basePath)
{
    if (m_initialized) {
        Logging::debug("Cache::CacheEntry", "Already initialized");
        return true;
    }

    Logging::info("Cache::CacheEntry",
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(QStringLiteral("."))) {
            Logging::error("Cache::CacheEntry",
...
bool CacheEntry::put(const QString &key, const QByteArray &data,
...
QByteArray CacheEntry::get(const QString &key) const
{
    if (!m_initialized) {
        Logging::error("Cache::CacheEntry",
...
bool CacheEntry::remove(const QString &key)
{
    if (!m_initialized)
        return false;
...
bool CacheEntry::contains(const QString &key) const
{
    if (!m_initialized)
        return false;
...
void CacheEntry::clear()
{
    ...
    emit cacheCleared();
    Logging::info("Cache::CacheEntry", "Cache cleared");
}

qint64 CacheEntry::totalSize() const
{
    if (!m_initialized)
        return 0;
...
qint64 CacheEntry::entryCount() const
{
    if (!m_initialized)
        return 0;
...
void CacheEntry::setMaxSize(qint64 maxSizeBytes)
{
    m_maxSize = qMax(static_cast<qint64>(1024 * 1024), maxSizeBytes);
    Logging::debug("Cache::CacheEntry",
        QString("Max size set to: %1 MB").arg(m_maxSize / (1024 * 1024)));
}

qint64 CacheEntry::maxSize() const
{
    return m_maxSize;
}

void CacheEntry::prune()
{
    if (!m_initialized)
        return;
...
void CacheEntry::close()
{
    ...
    Logging::info("Cache::CacheEntry", "Closing cache...");
    flush();
    m_initialized = false;
    Logging::info("Cache::CacheEntry", "Cache closed");
}

// Private helpers
QString CacheEntry::entryPath(const QString &key) const
{
