 ...
 * File: image_view_activity.cpp - Port of ImageViewActivity.java
 *
...
#include "image_view_activity.h"

namespace PinkReader {

// ===== Static constant (port of: private static final String TAG = "ImageViewActivity") =====
const QString ImageViewActivity::TAG = QStringLiteral("ImageViewActivity");

// ===== Constructor =====
ImageViewActivity::ImageViewActivity(QObject *parent)
    : QObject(parent)
{
}

// ===== Destructor =====
ImageViewActivity::~ImageViewActivity() = default;

// ===== Port of: protected boolean baseActivityIsToolbarActionBarEnabled() =====
bool ImageViewActivity::baseActivityIsToolbarActionBarEnabled() {
    // return false; (from Java source)
    return false;
}

// ===== Port of: protected void onCreate(final Bundle savedInstanceState) =====
void ImageViewActivity::onCreate(Bundle *savedInstanceState) {
    ...
    //     LinkHandler.getAlbumInfo(this, ...);
    // }
    //
    // // Create DonutProgress bar, LinearLayouts, RelativeLayout, etc.
    ...
    // }
    //
    // LinkHandler.getImageInfo(this, mUrl, new Priority(Constants.Priority.IMAGE_VIEW),
    //     new GetImageInfoListener() {
    //         @Override public void onFailure(@NonNull final RRError error) {
    //             General.quickToast(ImageViewActivity.this,
    //                 R.string.imageview_image_info_failed);
    //             revertToWeb();
    //         }
    //         @Override public void onSuccess(final ImageInfo info) {
    //             mImageInfo = info;
    ...
    //     });
    //
    // // Create RedditPreparedPost
    // final RedditPreparedPost post;
    // if(mPost != null) {
    //     final RedditParsedPost parsedPost = new RedditParsedPost(this, mPost, false);
    //     post = new RedditPreparedPost(this, CacheManager.getInstance(this), 0,
    //         parsedPost, TimestampUTC.ZERO, false, false, false, false);
    // } else {
    //     post = null;
    // }
    //
    // // Hidden accessibility layout
    ...
    //     if(post != null) {
    //         commentsButton.setOnClickListener(v -> RedditPostActions.INSTANCE
    //             .onActionMenuItemSelected(post, this,
    ...
    //                     R.layout.floating_toolbar, outerFrame, false));
    //     ...
...
// ===== Port of: private void setMainView(final View v) =====
void ImageViewActivity::setMainView(View *v) {
...
// ===== Port of: private void onImageStreamReady(
...
void ImageViewActivity::onImageStreamReady(bool isNetwork,
    ...
    //     }
    //
    //     if(mImageInfo != null && ((mImageInfo.title != null && ...) || ...)) {
    ...
    //     }
    //
    //     // Fully download before playing if configured
    //     final boolean fullyDownloadBeforePlaying = PrefsUtility.pref_videos_download_before_playing();
...
void ImageViewActivity::onPostSelected(RedditPreparedPost *post) {
...
void ImageViewActivity::onPostCommentsSelected(RedditPreparedPost *post) {
...
void ImageViewActivity::onBackPressed() {
...
// ===== Port of: private void revertToWeb() =====
void ImageViewActivity::revertToWeb() {
...
// ===== Port of: private void openInExternalBrowser() =====
void ImageViewActivity::openInExternalBrowser() {
...
// ===== Port of: public void onPause() =====
void ImageViewActivity::onPause() {
    // if(mIsPaused) {
...
// ===== Port of: public void onResume() =====
void ImageViewActivity::onResume() {
    // if(!mIsPaused) {
    //     throw new RuntimeException();
    // }
    //
...
// ===== Port of: public void onDestroy() =====
void ImageViewActivity::onDestroy() {
    // super.onDestroy();
    // mIsDestroyed = true;
    //
    // cancelCacheRequests();
...
// ===== Port of: private void cancelCacheRequests() =====
void ImageViewActivity::cancelCacheRequests() {
...
// ===== Port of: public void onSingleTap() =====
void ImageViewActivity::onSingleTap() {
...
// ===== Port of: public void onHorizontalSwipe(final float pixels) =====
void ImageViewActivity::onHorizontalSwipe(float pixels) {
    // if(mSwipeCancelled) {
    //     return;
    // }
    //
...
// ===== Port of: public void onHorizontalSwipeEnd() =====
void ImageViewActivity::onHorizontalSwipeEnd() {
...
// ===== Port of: public void onImageViewDLMOutOfMemory() =====
void ImageViewActivity::onImageViewDLMOutOfMemory() {
    // if(!mHaveReverted) {
...
// ===== Port of: public void onImageViewDLMException(final Throwable t) =====
void ImageViewActivity::onImageViewDLMException(void *t) {
    // if(!mHaveReverted) {
...
// ===== Port of: public void onConfigurationChanged(@NonNull final Configuration newConfig) =====
void ImageViewActivity::onConfigurationChanged(Configuration *newConfig) {
...
// ===== Port of: private void openImage(
//     final DonutProgress progressBar, final UriString uri, @Nullable final UriString audioUri) =====
void ImageViewActivity::openImage(DonutProgress *progressBar,
                                   UriString *uri,
                                   UriString *audioUri) {
...
// ===== Port of: private void manageAspectRatioIndicator(final DonutProgress progressBar) =====
void ImageViewActivity::manageAspectRatioIndicator(DonutProgress *progressBar) {
    // findAspectRatio:
...
// ===== Port of: private void makeCacheRequest(
//     final DonutProgress progressBar, final UriString uri, @Nullable final UriString audioUri) =====
void ImageViewActivity::makeCacheRequest(DonutProgress *progressBar,
                                          UriString *uri,
                                          UriString *audioUri) {
    // final Object resultLock = new Object();
    //
    // final AtomicBoolean failed = new AtomicBoolean(false);
    // final AtomicReference<GenericFactory<SeekableInputStream, IOException>> audio = new AtomicReference<>();
    // final AtomicReference<GenericFactory<SeekableInputStream, IOException>> video = new AtomicReference<>();
    ...
    //             }
    //
    //             @Override public void onProgress(final boolean authorizationInProgress,
    ...
    //                         onImageStreamReady(!fromCache, streamFactory, audio.get(), mimetype,
    ...
    //                     synchronized(resultLock) {
    //                         if(video.get() != null) {
    //                             onImageStreamReady(!fromCache, video.get(), streamFactory,
    //                                 videoMimetype.get(), Uri.parse(uri.toString()));
    //                         } else {
...
// ===== Port of: @Nullable private ImageButton addFloatingToolbarButton(
//     @DrawableRes final int drawable, @StringRes final int description,
//     @NonNull final View.OnClickListener listener) =====
ImageButton *ImageViewActivity::addFloatingToolbarButton(int drawable, int description,
...
// ===== Port of: private void launchVlc(@NonNull final Uri uri) =====
void ImageViewActivity::launchVlc(void *uri) {
...
// ===== Port of: private void playWithExoplayer(
//     final boolean isNetwork,
//     @NonNull final GenericFactory<SeekableInputStream, IOException> videoStream,
//     @Nullable final GenericFactory<SeekableInputStream, IOException> audioStream) =====
void ImageViewActivity::playWithExoplayer(bool isNetwork,
                                           GenericFactory *videoStream,
                                           GenericFactory *audioStream) {
    ...
    //     final MediaSource videoMediaSource = new ProgressiveMediaSource.Factory(videoDataSourceFactory)
    ...
    //     }
    //
    //     mVideoPlayerWrapper = new ExoPlayerWrapperView(this, mediaSource,
    //             this::revertToWeb, 0);
    //
    ...
    //
    //     final BasicGestureHandler gestureHandler = new BasicGestureHandler(this);
...
// ===== Port of: private void playGIFWithMovie(
//     @NonNull final GenericFactory<SeekableInputStream, IOException> streamFactory) =====
void ImageViewActivity::playGIFWithMovie(GenericFactory *streamFactory) {
...
// ===== Port of: private void playGIFWithLegacyDecoder(
//     @NonNull final GenericFactory<SeekableInputStream, IOException> streamFactory) =====
void ImageViewActivity::playGIFWithLegacyDecoder(GenericFactory *streamFactory) {
    ...
    // }
    //
    // gifThread = new GifDecoderThread(is, new GifDecoderThread.OnGifLoadedListener() {
    //     @Override public void onGifLoaded() {
    //         AndroidCommon.UI_THREAD_HANDLER.post(() -> {
    //             if(mIsDestroyed) return;
    ...
    //         });
    //     }
    //     @Override public void onOutOfMemory() {
    //         General.quickToast(ImageViewActivity.this, R.string.imageview_oom);
    //         revertToWeb();
    //     }
    //     @Override public void onGifInvalid() {
...
// ===== Port of: private void showImageWithInternalViewer(
//     @NonNull final GenericFactory<SeekableInputStream, IOException> streamFactory) =====
void ImageViewActivity::showImageWithInternalViewer(GenericFactory *streamFactory) {
    ...
    // AndroidCommon.UI_THREAD_HANDLER.post(() -> {
    //     if(mIsDestroyed) return;
    //     mImageViewDisplayerManager = new ImageViewDisplayListManager(imageTileSource, this);
