 ...
 * GPLv3 License
 * File: font_manager.cpp - Implementation of Font scaling and configuration
 */
...
{
    if (m_loaded) {
        Logging::debug("Settings::FontManager", "Already loaded");
        return;
    }

    Logging::info("Settings::FontManager", "Loading settings...");
    ...
    Logging::info("Settings::FontManager",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void FontManager::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}
...
{
    Logging::info("Settings::FontManager", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString FontManager::getString(const QString &key,
...
bool FontManager::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int FontManager::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double FontManager::getDouble(const QString &key,
...
QByteArray FontManager::getByteArray(const QString &key,
...
void FontManager::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void FontManager::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void FontManager::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void FontManager::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void FontManager::setByteArray(const QString &key,
...
bool FontManager::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void FontManager::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList FontManager::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject FontManager::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool FontManager::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::FontManager",
...
void FontManager::registerDefaults()
{
