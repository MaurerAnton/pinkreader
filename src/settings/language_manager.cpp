 ...
 * GPLv3 License
 * File: language_manager.cpp - Implementation of Language and translation management
 */
...
{
    if (m_loaded) {
        Logging::debug("Settings::LanguageManager", "Already loaded");
        return;
    }

    Logging::info("Settings::LanguageManager", "Loading settings...");
    ...
    Logging::info("Settings::LanguageManager",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void LanguageManager::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}

void LanguageManager::resetToDefaults()
{
    Logging::info("Settings::LanguageManager", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString LanguageManager::getString(const QString &key,
...
bool LanguageManager::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int LanguageManager::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double LanguageManager::getDouble(const QString &key,
...
QByteArray LanguageManager::getByteArray(const QString &key,
...
void LanguageManager::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void LanguageManager::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void LanguageManager::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void LanguageManager::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void LanguageManager::setByteArray(const QString &key,
...
bool LanguageManager::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void LanguageManager::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList LanguageManager::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject LanguageManager::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool LanguageManager::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::LanguageManager",
...
void LanguageManager::registerDefaults()
{
