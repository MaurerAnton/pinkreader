 ...
 * GPLv3 License
 * File: preferences.cpp - Implementation of Persistent user preferences manager
 */
...
{
    if (m_loaded) {
        Logging::debug("Settings::Preferences", "Already loaded");
        return;
    }

    Logging::info("Settings::Preferences", "Loading settings...");
    ...
    Logging::info("Settings::Preferences",
...
{
    Logging::info("Settings::Preferences", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString Preferences::getString(const QString &key,
...
bool Preferences::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int Preferences::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double Preferences::getDouble(const QString &key,
...
QByteArray Preferences::getByteArray(const QString &key,
...
void Preferences::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void Preferences::setByteArray(const QString &key,
...
QStringList Preferences::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject Preferences::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool Preferences::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::Preferences",
...
void Preferences::registerDefaults()
{
