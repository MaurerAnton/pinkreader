 ...
 * GPLv3 License
 * File: cache_settings.cpp - Implementation of Cache behavior settings
 */
...
CacheSettings::CacheSettings(QObject *parent)
    : QObject(parent)
{
...
void CacheSettings::load()
{
    if (m_loaded) {
        Logging::debug("Settings::CacheSettings", "Already loaded");
        return;
    }

    Logging::info("Settings::CacheSettings", "Loading settings...");
    ...
    Logging::info("Settings::CacheSettings",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void CacheSettings::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}

void CacheSettings::resetToDefaults()
{
    Logging::info("Settings::CacheSettings", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString CacheSettings::getString(const QString &key,
...
bool CacheSettings::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int CacheSettings::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double CacheSettings::getDouble(const QString &key,
...
QByteArray CacheSettings::getByteArray(const QString &key,
...
void CacheSettings::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void CacheSettings::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void CacheSettings::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void CacheSettings::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void CacheSettings::setByteArray(const QString &key,
...
bool CacheSettings::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void CacheSettings::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList CacheSettings::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject CacheSettings::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool CacheSettings::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::CacheSettings",
...
void CacheSettings::registerDefaults()
{
