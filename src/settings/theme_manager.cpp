 ...
 * GPLv3 License
 * File: theme_manager.cpp - Implementation of Application theme management
 */
...
{
    if (m_loaded) {
        Logging::debug("Settings::ThemeManager", "Already loaded");
        return;
    }

    Logging::info("Settings::ThemeManager", "Loading settings...");
    ...
    Logging::info("Settings::ThemeManager",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void ThemeManager::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}
...
{
    Logging::info("Settings::ThemeManager", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString ThemeManager::getString(const QString &key,
...
bool ThemeManager::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int ThemeManager::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double ThemeManager::getDouble(const QString &key,
...
QByteArray ThemeManager::getByteArray(const QString &key,
...
void ThemeManager::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void ThemeManager::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemeManager::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemeManager::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemeManager::setByteArray(const QString &key,
...
bool ThemeManager::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void ThemeManager::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList ThemeManager::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject ThemeManager::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool ThemeManager::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::ThemeManager",
...
void ThemeManager::registerDefaults()
{
