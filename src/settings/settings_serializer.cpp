 ...
 * GPLv3 License
 * File: settings_serializer.cpp - Implementation of Settings backup and restore
 */

#include "settings/settings_serializer.h"
...
SettingsSerializer::SettingsSerializer(QObject *parent)
    : QObject(parent)
{
...
{
    if (m_loaded) {
        Logging::debug("Settings::SettingsSerializer", "Already loaded");
        return;
    }

    Logging::info("Settings::SettingsSerializer", "Loading settings...");
    ...
    Logging::info("Settings::SettingsSerializer",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void SettingsSerializer::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}

void SettingsSerializer::resetToDefaults()
{
    Logging::info("Settings::SettingsSerializer", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString SettingsSerializer::getString(const QString &key,
...
bool SettingsSerializer::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int SettingsSerializer::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double SettingsSerializer::getDouble(const QString &key,
...
QByteArray SettingsSerializer::getByteArray(const QString &key,
...
void SettingsSerializer::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void SettingsSerializer::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void SettingsSerializer::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void SettingsSerializer::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void SettingsSerializer::setByteArray(const QString &key,
...
bool SettingsSerializer::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void SettingsSerializer::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList SettingsSerializer::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject SettingsSerializer::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool SettingsSerializer::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::SettingsSerializer",
...
void SettingsSerializer::registerDefaults()
{
