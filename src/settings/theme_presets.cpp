 ...
 * GPLv3 License
 * File: theme_presets.cpp - Implementation of Built-in theme presets
 */

#include "settings/theme_presets.h"
...
void ThemePresets::load()
{
    if (m_loaded) {
        Logging::debug("Settings::ThemePresets", "Already loaded");
        return;
    }

    Logging::info("Settings::ThemePresets", "Loading settings...");
    ...
    Logging::info("Settings::ThemePresets",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void ThemePresets::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}

void ThemePresets::resetToDefaults()
{
    Logging::info("Settings::ThemePresets", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString ThemePresets::getString(const QString &key,
...
bool ThemePresets::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int ThemePresets::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double ThemePresets::getDouble(const QString &key,
...
QByteArray ThemePresets::getByteArray(const QString &key,
...
void ThemePresets::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void ThemePresets::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemePresets::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemePresets::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void ThemePresets::setByteArray(const QString &key,
...
bool ThemePresets::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->contains(key);
}

void ThemePresets::remove(const QString &key)
{
    QMutexLocker locker(&m_mutex);
    m_settings->remove(key);
}

QStringList ThemePresets::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject ThemePresets::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool ThemePresets::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::ThemePresets",
...
void ThemePresets::registerDefaults()
{
