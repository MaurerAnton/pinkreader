 ...
 * GPLv3 License
 * File: notification_settings.cpp - Implementation of Notification and inbox settings
 */

#include "settings/notification_settings.h"
...
{
    if (m_loaded) {
        Logging::debug("Settings::NotificationSettings", "Already loaded");
        return;
    }

    Logging::info("Settings::NotificationSettings", "Loading settings...");
    ...
    Logging::info("Settings::NotificationSettings",
        QString("Settings loaded (%1 keys)").arg(allKeys().size()));
}

void NotificationSettings::sync()
{
    QMutexLocker locker(&m_mutex);
    m_settings->sync();
}

void NotificationSettings::resetToDefaults()
{
    Logging::info("Settings::NotificationSettings", "Resetting to defaults...");
    QMutexLocker locker(&m_mutex);
...
QString NotificationSettings::getString(const QString &key,
...
bool NotificationSettings::getBool(const QString &key, bool defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toBool();
}

int NotificationSettings::getInt(const QString &key, int defaultValue) const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->value(key, defaultValue).toInt();
}

double NotificationSettings::getDouble(const QString &key,
...
QByteArray NotificationSettings::getByteArray(const QString &key,
...
void NotificationSettings::setString(const QString &key, const QString &value)
{
    {
        QMutexLocker locker(&m_mutex);
...
void NotificationSettings::setBool(const QString &key, bool value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void NotificationSettings::setInt(const QString &key, int value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void NotificationSettings::setDouble(const QString &key, double value)
{
    {
        QMutexLocker locker(&m_mutex);
        m_settings->setValue(key, value);
...
void NotificationSettings::setByteArray(const QString &key,
...
bool NotificationSettings::contains(const QString &key) const
{
    QMutexLocker locker(&m_mutex);
...
QStringList NotificationSettings::allKeys() const
{
    QMutexLocker locker(&m_mutex);
    return m_settings->allKeys();
}

QJsonObject NotificationSettings::exportAll() const
{
    QMutexLocker locker(&m_mutex);
    QJsonObject obj;
...
bool NotificationSettings::importAll(const QJsonObject &settings)
{
    Logging::info("Settings::NotificationSettings",
...
void NotificationSettings::registerDefaults()
{
