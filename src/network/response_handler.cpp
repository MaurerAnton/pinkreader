 ...
 * Description: Implementation of API response parsing and error handling
 */

#include "network/response_handler.h"
...
bool ResponseHandler::initialize()
{
    if (m_initialized) {
        Logging::debug("Network::" + QString::fromLatin1("ResponseHandler"),
            "Already initialized, skipping");
        return true;
    }

    Logging::info("Network::" + QString::fromLatin1("ResponseHandler"),
        "Initializing ResponseHandler...");

    // Validate configuration
    if (m_baseUrl.isEmpty()) {
        Logging::error("Network::" + QString::fromLatin1("ResponseHandler"),
...
bool ResponseHandler::isInitialized() const
{
    return m_initialized;
}

void ResponseHandler::get(const QString &endpoint,
...
void ResponseHandler::post(const QString &endpoint,
...
void ResponseHandler::put(const QString &endpoint,
                        const QJsonObject &body,
                        ResponseCallback callback)
{
    if (!m_initialized) {
...
void ResponseHandler::deleteRequest(const QString &endpoint,
                                  ResponseCallback callback)
{
    if (!m_initialized) {
        Logging::error("Network::" + QString::fromLatin1("ResponseHandler"),
...
void ResponseHandler::setBaseUrl(const QString &url)
{
    m_baseUrl = url;
    Logging::debug("Network::" + QString::fromLatin1("ResponseHandler"),
...
QString ResponseHandler::baseUrl() const
{
    return m_baseUrl;
}

void ResponseHandler::setTimeout(int milliseconds)
{
...
int ResponseHandler::timeout() const
{
    return m_timeout;
}

void ResponseHandler::setMaxRetries(int retries)
{
...
int ResponseHandler::maxRetries() const
{
    return m_maxRetries;
}

int ResponseHandler::pendingRequestCount() const
{
    QMutexLocker locker(&m_requestMutex);
    return m_activeReplies.size() + m_pendingRequests.size();
}

int ResponseHandler::totalRequestsSent() const
{
    QMutexLocker locker(&m_statsMutex);
    return m_totalSent;
}

int ResponseHandler::totalRequestsFailed() const
{
    QMutexLocker locker(&m_statsMutex);
    return m_totalFailed;
}

void ResponseHandler::clearStatistics()
{
    QMutexLocker locker(&m_statsMutex);
    m_totalSent = 0;
    m_totalFailed = 0;
...
void ResponseHandler::executeRequest(const RequestContext &ctx)
{
    // Build the full URL
...
void ResponseHandler::handleReply(QNetworkReply *reply, RequestContext ctx)
{
    reply->deleteLater();
...
void ResponseHandler::retryRequest(RequestContext ctx)
{
    ctx.retryCount++;
...
bool ResponseHandler::shouldRetry(int statusCode) const
{
