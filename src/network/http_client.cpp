 ...
 * Description: Implementation of HTTP client wrapper around Qt Network
 */
...
{
    if (m_initialized) {
        Logging::debug("Network::" + QString::fromLatin1("HttpClient"),
            "Already initialized, skipping");
        return true;
    }

    Logging::info("Network::" + QString::fromLatin1("HttpClient"),
        "Initializing HttpClient...");

    // Validate configuration
    if (m_baseUrl.isEmpty()) {
        Logging::error("Network::" + QString::fromLatin1("HttpClient"),
...
void HttpClient::put(const QString &endpoint,
                        const QJsonObject &body,
                        ResponseCallback callback)
{
    if (!m_initialized) {
...
int HttpClient::maxRetries() const
{
    return m_maxRetries;
}

int HttpClient::pendingRequestCount() const
{
...
int HttpClient::totalRequestsFailed() const
{
    QMutexLocker locker(&m_statsMutex);
...
void HttpClient::executeRequest(const RequestContext &ctx)
{
    // Build the full URL
...
void HttpClient::handleReply(QNetworkReply *reply, RequestContext ctx)
{
    reply->deleteLater();
...
void HttpClient::retryRequest(RequestContext ctx)
{
    ctx.retryCount++;
