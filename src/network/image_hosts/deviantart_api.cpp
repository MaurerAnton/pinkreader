 ...
 * ... (full GPLv3 license) ...
 *
 * File: deviantart_api.cpp
 * Description: Implementation of DeviantArt image support
 */

#include "network/image_hosts/deviantart_api.h"
...
DeviantartApi::DeviantartApi(QObject *parent)
    : QObject(parent)
{
    Logging::debug("ImageHosts",
        QString("Created DeviantartApi handler"));
}

DeviantartApi::~DeviantartApi()
{
    Logging::debug("ImageHosts",
        QString("Destroyed DeviantartApi handler"));
}

bool DeviantartApi::canHandleUrl(const QUrl &url) const
{
    if (!m_enabled)
        return false;
...
QString DeviantartApi::hostName() const
{
    return QStringLiteral("deviantart_api");
}

void DeviantartApi::fetchImages(const QUrl &url,
                                 ImageCallback callback)
{
    if (!m_enabled) {
        Logging::debug("ImageHosts",
            QString("DeviantartApi is disabled"));
        if (callback)
    ...
    Logging::info("ImageHosts",
        QString("Fetching images from DeviantartApi: %1").arg(url.toString()));
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, callback]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            ...
            Logging::error("ImageHosts",
                QString("DeviantartApi fetch failed: %1").arg(error));
        ...
        if (parseError.error != QJsonParseError::NoError) {
            Logging::error("ImageHosts",
                QString("DeviantartApi JSON parse error: %1")
        ...
        Logging::info("ImageHosts",
            QString("DeviantartApi fetched %1 images").arg(images.size()));
...
void DeviantartApi::fetchImageInfo(const QUrl &url,
                                    ImageCallback callback)
{
...
QUrl DeviantartApi::directUrl(const QUrl &url) const
{
    // Default: return the URL as-is
    return url;
}

void DeviantartApi::setApiKey(const QString &key)
{
    m_apiKey = key;
    Logging::debug("ImageHosts",
        QString("DeviantartApi API key %1")
            .arg(key.isEmpty() ? QStringLiteral("cleared")
                              : QStringLiteral("set")));
}

QString DeviantartApi::apiKey() const
{
    return m_apiKey;
}

void DeviantartApi::setEnabled(bool enabled)
{
    m_enabled = enabled;
    Logging::debug("ImageHosts",
        QString("DeviantartApi %1")
            .arg(enabled ? QStringLiteral("enabled")
                        : QStringLiteral("disabled")));
}

bool DeviantartApi::isEnabled() const
{
    return m_enabled;
}
